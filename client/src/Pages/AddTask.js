...
const AddTask = () => {
    ...
    function getFetchTaskDetails() {
        axios.get(`http://localhost:3001/api/tasks/${params.id}`)
            .then(res => {
                if (res.status === 200) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (params.id) {
            axios.put(`http://localhost:3001/api/tasks/${params.id}`, {
                title,
                description,
    ...
    const handleDelete = () => {
        ...
        confirm({ description: "This Task will be permanently deleted after this action" })
            .then(() => {
                axios.delete(`http://localhost:3001/api/tasks/${params.id}`)
                    .then(res => {
                        if (res.status === 200) {
